...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// 3Dモデルのアニメーション関連を読み込む
	int animationIndex[Animation::Length];			// アニメーション番号
	for(int i=0; i<Animation::Length; i++)
	{
		// 3Dモデルのアニメーション番号読み込み
		animationIndex[i] = MV1GetAnimIndex(
			modelHandle, animationName[i]);
	}
	...
	// 待機アニメーションをアタッチする
	int attachIndex = MV1AttachAnim(modelHandle,
		animationIndex[animationNowIndex],
		-1, FALSE);

	// 待機アニメーションの総時間を取得しておく
	float animationTotalTime = MV1GetAttachAnimTotalTime(
		modelHandle, attachIndex);

	// アニメーション再生時間を初期化
	float animationNowTime = 0.0f;
	MV1SetAttachAnimTime(modelHandle, attachIndex,
		animationNowTime);
