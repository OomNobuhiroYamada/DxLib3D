...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	while(ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		// カメラの位置と向きを設定
		{
			...
			tempPosition2.x = CAMERA_LOOK_AT_DISTANCE * cosf(cameraVAngle / 180.0f * DX_PI_F) * sinf(cameraHAngle / 180.0f * DX_PI_F);
			tempPosition2.y = CAMERA_LOOK_AT_DISTANCE * sinf(cameraVAngle / 180.0f * DX_PI_F);
			tempPosition2.z = -(CAMERA_LOOK_AT_DISTANCE * cosf(cameraVAngle / 180.0f * DX_PI_F) * cosf(cameraHAngle / 180.0f * DX_PI_F));
