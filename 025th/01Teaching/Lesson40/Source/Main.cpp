...
* @date 2023/01/09
*
* @details 3Dアクション基本4（装飾物をキャラクタに付ける）
...
void Player_Initialize(void)
{
	...
	player.stickModelHandle = MV1LoadModel("Resource/Stick.mqo");
	player.hatModelHandle = MV1LoadModel("Resource/Hat.mqo");
...
void Player_Terminate()
{
	...
	MV1DeleteModel(player.stickModelHandle);
	MV1DeleteModel(player.hatModelHandle);
...
/**
* @fn SetModelFramePosition
* @brief あるモデルの指定のフレームの位置に別のモデルを設置する
* @param[in] int modelHandle, const char *frameName, int setModelHandle
*/
...
void Player_Process()
{
	...
	// 棒モデルを右腕の指の場所に設置する
	SetModelFramePosition(player.modelHandle, "FingerR", player.stickModelHandle);

	// 帽子モデルを頭の場所に設置する
	SetModelFramePosition(player.modelHandle, "Head", player.hatModelHandle);
}
...
void Player_Move(VECTOR moveVector)
{
	...
	// ( 検出する範囲は移動距離も考慮する )
	hitDim = MV1CollCheck_Sphere(stage.modelHandle, -1, player.position, CHARA_ENUM_DEFAULT_SIZE + VSize(moveVector));

	// x軸かy軸方向に 0.01f 以上移動した場合は「移動した」フラグをtrueにする
...
void Player_PlayAnim(int playAnim)
{
	...
	player.playAnim1 = MV1AttachAnim(player.modelHandle, playAnim);
...
void Player_ShadowRender()
{
	...
	hitResDim = MV1CollCheck_Capsule(stage.modelHandle, -1, player.position, VAdd(player.position, VGet(0.0f, -CHARA_SHADOW_HEIGHT, 0.0f)), CHARA_SHADOW_SIZE);
...
void Render_Process()
{
	...
	// 棒モデルの描画
	MV1DrawModel(player.stickModelHandle);

	// 帽子モデルの描画
	MV1DrawModel(player.hatModelHandle);
